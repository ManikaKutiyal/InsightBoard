...
const SidebarItem = ({ emoji, label, active }) => (
  <div
    ...
    ${active ? "bg-brand-coral text-white shadow-lg" : "text-brand-charcoal hover:bg-white"}`}
  >
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* DESKTOP SIDEBAR */}
      <aside className="hidden lg:flex w-28 min-h-screen flex-col items-center py-8 border-r border-brand-border/50 bg-brand-bg sticky top-0 overflow-y-auto">
        <div className="mb-12 text-4xl">🎀</div>
        <nav className="flex-1">
          <NavContent />
        </nav>
        <div className="text-[10px] font-black text-brand-coral rotate-90 mb-4 opacity-50 tracking-widest">
          EST. 2024
        </div>
      </aside>

      {/* MOBILE BOTTOM BAR */}
      <nav className="lg:hidden fixed bottom-0 left-0 w-full bg-white/80 backdrop-blur-xl border-t border-brand-border/30 flex justify-around items-center py-2 px-4 z-50">
        <NavContent />
      </nav>
    </>
  );
